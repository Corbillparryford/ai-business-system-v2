...
def _generate_content(legs: list[dict], combined_odds: int) -> dict:
    ...
    prompt = (
        "You are writing social media content for a daily sports parlay.\n\n"
        f"Parlay legs:\n{leg_summary}\n"
        f"Combined odds: +{combined_odds:,}\n\n"
        "Parlay strategy context:\n"
        "- 3–4 legs\n"
        "- Prefer slight positive EV, odds between -150 and +200 per leg\n"
        "- Target +1000 to +3000 combined odds\n"
        "- This is an engagement play, not core strategy\n\n"
        "Write the following sections with NO extra text outside them:\n\n"
        "IMAGE PROMPT:\n"
        "Dark cinematic background, professional sports poster. "
        "1-2 star players from key legs facing each other, subtle glow. Faint stadium midground. "
        "Overlay: PARLAY OF THE DAY, odds displayed, legs listed cleanly. "
        "Square format, minimal clutter, premium typography, THE SHARP MARGIN branding.\n\n"
        "INSTAGRAM:\n"
        "3-5 short lines. Natural tone, calm confidence. "
        "Mention the odds naturally. "
        "Note at the end that this is for engagement, not core strategy. "
        "End with https://whop.com/@thesharpmargin\n"
        "Style: 'Parlay for today. Few spots we like across games. "
        "Comes out to +1800. Not a core play — just something for the slate.'\n\n"
        "TWITTER:\n"
        "1-2 lines. Mention odds. End with https://whop.com/@thesharpmargin\n\n"
        "TONE: Human, calm, no hype. Sound like someone posting a casual play, not marketing."
    )
...
def generate_daily_parlay(opportunities: list) -> dict | None:
    ...
    if combined_odds < 1000:
        log.info("Combined odds +%d below +1000 target — posting anyway", combined_odds)
