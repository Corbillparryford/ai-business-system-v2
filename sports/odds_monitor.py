        ...
        try:
            ...
            if opportunities:
                ...
                if cycle_count % SPORTS_CLAUDE_EVERY_N_CYCLES != 0:
                    ...
                    # ── Pre-Claude filter: split EV and ARB, apply per-type gates ──
                    ev_opps:  list = []
                    arb_opps: list = []
                    ...
                    for opp in opportunities:
                        ...
                        # ── Edge pre-filter (applied to all types) ────────────
                        if edge < 1.0 or edge > 15.0:
                            log.debug("Skipped %s %s — edge %.2f%% out of range [1,15]",
                        ...
                        if opp_type == "POSITIVE_EV":
                            try:
                                ...
                                timing_min = 999   # unparseable → pass through

                            if timing_min < 0:
                                log.info("Skipped EV %s — timing %.0f min (game already started)",
                                         matchup, timing_min)
                                continue
                            if timing_min > 1440:
                                log.info("Skipped EV %s — timing %.0f min (too far in future)",
                                         matchup, timing_min)
                                continue
                            ev_opps.append(opp)
                    ...
                    log.info("Valid EV opportunities: %d", len(ev_opps))
                    log.info("Valid ARB opportunities: %d", len(arb_opps))

                    # ── Fallback: if no EV passed filters, use top-3 by edge ──
                    if not ev_opps and opportunities:
                        fallback = sorted(
                            [o for o in opportunities
                             if float(o.get("edge", o.get("arb_percentage", 0))) <= 20.0],
                            key=lambda o: float(o.get("edge", o.get("arb_percentage", 0))),
                            reverse=True,
                        )[:3]
                        if fallback:
                            ev_opps = fallback
                            log.info("Using fallback opportunities (%d)", len(ev_opps))
                    ...
                    if not filtered_opps:
                        log.debug("All opportunities filtered out — skipping Claude")
                    else:
                        best_edge = max(
