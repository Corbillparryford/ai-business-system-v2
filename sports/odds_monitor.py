                ...
                if cycle_count % SPORTS_CLAUDE_EVERY_N_CYCLES != 0:
                    ...
                    for opp in opportunities:
                        ...
                        if opp_type == "POSITIVE_EV":
                            # Parse timing from the "X min to start" string
                            try:
                                timing_min = float(str(opp.get("timing", "0")).split()[0])
                            except (ValueError, IndexError):
                                timing_min = -1

                            if timing_min < 2:
                                log.info("Skipped EV %s — timing %.0f min (game started or too close)",
                                         matchup, timing_min)
                                continue
                            if timing_min > 1440:
                            ...
                            if edge < 1.0 or edge > 15.0:
                                log.info("Skipped ARB %s — edge %.2f%% invalid", matchup, edge)
                                continue
                            arb_opps.append(opp)

                        else:
                            # Cross-market / Polymarket — apply same EV timing rules
                            try:
                                timing_min = float(str(opp.get("timing", "99999")).split()[0])
                            except (ValueError, IndexError):
                                timing_min = 99999
                            if timing_min >= 2:
                                ev_opps.append(opp)
