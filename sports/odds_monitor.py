...
def _resolve_results():
    ...
    for sig in active:
        ...
        for game in scores:
            ...
            record_result(result)             # daily win/loss counter
            record_bet_result(result)         # bankroll simulation
...
def run_odds_monitor():
    ...
    cycle_count           = 0
    idle_cycles           = 0
    ODDS_FETCH_EVERY_N    = 5   # fetch fresh odds every 5 cycles × 120s = every 10 min
    _last_sb_games: list  = []  # reuse between fetch cycles

    while True:
        loop_start     = time.time()
        opportunities  = []
        cycle_count   += 1
        sleep_interval = SPORTS_LOOP_SECONDS   # always defined — prevents crash if exception fires

        try:
            # ── Odds fetch gate: only call API every N cycles ─────────────────
            if cycle_count % ODDS_FETCH_EVERY_N == 0 or cycle_count == 1:
                ...
                log.debug("Skipping API call — using cached games (cycle %d)", cycle_count)

            poly_mkts    = fetch_polymarket_sports()   # has its own 5-min cache
            ...
            # Daily parlay at 12:00 UTC — uses current opportunity pool
