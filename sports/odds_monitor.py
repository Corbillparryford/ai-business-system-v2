...
_startup_message_sent: bool        = False  # fires once per process lifetime
...
def _soft_book_avg_odds(books: dict, team: str) -> float | None:
    """Average odds for a team across all soft books. Used when Pinnacle unavailable."""
    prices = []
    ...
    if not prices:
        return None
    # Convert to decimal, average, convert back to american
    decimals = [_american_to_decimal(p) for p in prices]
...
def _scan_ev(game: dict) -> list:
    ...
    # ── Try Pinnacle for true probabilities (ideal) ───────────────────────────
    pinnacle_h2h = {}
    ...
    if home in pinnacle_h2h and away in pinnacle_h2h:
        true_h, true_a = remove_vig(pinnacle_h2h[home], pinnacle_h2h[away])
    else:
        # ── Pinnacle unavailable: use soft-book average as proxy ───────────────
        avg_h = _soft_book_avg_odds(books, home)
        avg_a = _soft_book_avg_odds(books, away)
        if avg_h is None or avg_a is None:
            return []
        # Remove vig from average soft lines to approximate true probability
        avg_h_amer = int((avg_h - 1) * 100) if avg_h >= 2.0 else int(-100 / (avg_h - 1))
        avg_a_amer = int((avg_a - 1) * 100) if avg_a >= 2.0 else int(-100 / (avg_a - 1))
        true_h, true_a = remove_vig(avg_h_amer, avg_a_amer)
...
def _local_validate(signal: dict) -> bool:
    ...
    EV bets:  edge 1–15%, timing 0–1440 min (24h)
    ARB bets: edge 1–15%, timing 0–2880 min (48h)
    """
    # ── Edge (applies to all types) ───────────────────────────────────────────
    try:
    ...
    is_arb = (signal.get("type") or "").upper() == "ARBITRAGE"

    if is_arb:
        max_timing = 2880   # ARB: up to 48 hours
    else:
        max_timing = 1440   # EV: up to 24 hours

    if timing < 0:
        log.info("Signal rejected — timing %d min (game already started)", timing)
        return False
    if timing > max_timing:
        log.info("Signal rejected — timing %d min exceeds %dh window for %s",
                 timing, max_timing // 60, "ARB" if is_arb else "EV")
        return False
    ...
    for sig in active:
        ...
        for game in scores:
            ...
            if result == "WIN" and not (sig.get("signal_type") or "").upper() == "ARBITRAGE":
                ...
                # Marketing post for meaningful wins
                try:
                    from content.whop import post_win_marketing
                    from sports.performance_tracker import get_performance
                    bk_change = get_performance().get("bankroll_pct", 0.0)
                    post_win_marketing(
                        play            = sig.get("play", ""),
                        edge            = float(sig.get("edge_pct", sig.get("edge", 0))),
                        bankroll_change = bk_change,
                    )
                except Exception as e:
                    log.debug("Win marketing post failed: %s", e)
            break
...
def run_odds_monitor():
    ...
    # Send startup message exactly once per process lifetime
    if not _startup_message_sent:
        try:
            send(WEBHOOKS.get("health", ""), "✅ Sports engine started — system is working")
    ...
    while True:
        ...
        try:
            ...
            # If fingerprint cache blocked everything, force-scan all games
            if not opportunities and sb_games:
                log.info("Fingerprint cache blocked all games — force scanning all %d games", len(sb_games))
                for game in sb_games:
                    opportunities.extend(_scan_ev(game))
                    opportunities.extend(_scan_arb(game))
            ...
            log.info("Total opportunities: %d", len(opportunities))
            ...
            if opportunities:
                ...
                if cycle_count % SPORTS_CLAUDE_EVERY_N_CYCLES != 0:
                    ...
                    for opp in opportunities:
                        ...
                        edge      = float(opp.get("edge", opp.get("arb_percentage", 0)))

                        # Edge pre-filter
                        if edge < 1.0 or edge > 15.0:
                            log.debug("Skipped %s %s — edge %.2f%% out of range",
                                      opp_type, matchup, edge)
                            continue

                        if opp_type == "POSITIVE_EV":
                            try:
                                timing_min = float(str(opp.get("timing", "999")).split()[0])
                            except (ValueError, IndexError):
                                timing_min = 999

                            if timing_min < 0:
                                log.info("Skipped EV %s — timing %.0f min (game started)",
                                         matchup, timing_min)
                                continue
                            if timing_min > 1440:
                                log.info("Skipped EV %s — timing %.0f min (too far)", matchup, timing_min)
                                continue
                            ev_opps.append(opp)
                    ...
                    log.info("EV opportunities: %d", len(ev_opps))
                    log.info("ARB opportunities: %d", len(arb_opps))

                    # ── Fallback: always send SOMETHING to Claude ─────────────
                    if not ev_opps and opportunities:
                        log.info("⚠️ Using fallback opportunities")
                        fallback = sorted(
                            opportunities,
                    ...
                    filtered_opps = ev_opps + arb_opps

                    if not filtered_opps:
                        log.warning("🚨 No opportunities even after fallback")
                    else:
                        log.info("Filtered: %d EV + %d ARB = %d total (from %d raw)",
                                 len(ev_opps), len(arb_opps), len(filtered_opps), len(opportunities))
                        ...
                        log.info("Sending %d opportunities to Claude", len(limited))
                        result = call_betting_brain(limited, data_quality)

                        raw_signals = result.get("signals", [])
                        log.info("Claude returned %d signal(s)", len(raw_signals))
                        log.info("Total raw opportunities: %d", len(opportunities))

                        if not raw_signals:
                            log.info("Claude found no qualifying signals in this batch "
                                     "(opportunities may not clear confidence threshold)")

                        posted_count = 0
                        for signal in raw_signals:
                            if not _local_validate(signal):
                                log.info("Signal dropped by _local_validate: %s edge=%s timing=%s",
                                         signal.get("matchup", "?"),
                                         signal.get("edge", "?"),
                                         signal.get("timing", "?"))
                                continue
                            ...
                            if not check.get("approved", True):
                                log.info("Signal rejected by validate_signal: %s — %s",
                                         signal.get("matchup", "?"), check.get("reason", ""))
                                continue
                        ...
                        log.info("Valid signals count: %d  |  Signals posted: %d",
                                 posted_count, posted_count)

                        # If every signal was rejected, send a single note to #system-health
                        # (NOT to member channels — this is internal monitoring only)
                        if raw_signals and posted_count == 0:
                            send(
                                WEBHOOKS.get("health", ""),
                                "ℹ️ Signals generated but all rejected by validation "
                                "(timing or edge out of range). Monitoring continues.",
                            )
            else:
                idle_cycles += 1
            ...
            if now_utc.hour == 23:
                ...
                try:
                    ...
                    pass
                # Marketing recap to Whop
                try:
                    from content.whop import post_daily_recap_whop
                    from sports.results_tracker import get_results
                    from sports.performance_tracker import get_performance
                    r   = get_results()
                    bk  = get_performance().get("bankroll_pct", 0.0)
                    arb = any(b.get("is_arb") for b in _daily_bets)
                    post_daily_recap_whop(
                    ...
                    log.debug("Daily recap Whop post failed: %s", e)
