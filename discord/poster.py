...
from core.config import (
    ...
    DISCORD_WEBHOOK_HEALTH, DISCORD_WEBHOOK_CONTENT,
    CACHE_FILE,
...
def _american_to_decimal(odds) -> float:
    """Convert american odds (int or str like '+145' / '-110') to decimal."""
    try:
        o = int(str(odds).replace("+", "").strip())
        return (o / 100 + 1.0) if o > 0 else (100 / abs(o) + 1.0)
    except (ValueError, TypeError):
        return 2.0   # safe fallback (even money)
...
def _fmt_arb(signal: dict) -> str:
    ...
    # ── Build per-leg blocks with stake % ────────────────────────────────────
    # Stake split: each leg's weight = 1/decimal_odds, normalised to 100%
    if legs:
        decimals = [_american_to_decimal(leg.get("odds", 2)) for leg in legs]
        invs     = [1.0 / d for d in decimals]
        total    = sum(invs) or 1.0
        stakes   = [round(inv / total * 100, 1) for inv in invs]

        # Adjust for rounding so they sum to exactly 100%
        diff = round(100.0 - sum(stakes), 1)
        stakes[0] = round(stakes[0] + diff, 1)

        leg_lines = []
        for i, leg in enumerate(legs):
            c      = colours[i] if i < len(colours) else "⚪"
            side   = leg.get("side") or leg.get("play", "?")
            book   = leg.get("book", "?")
            odds   = leg.get("odds", "?")
            stake  = stakes[i]
            leg_lines.append(
        ...
        # Flat fallback when no legs array — parse from book/odds fields
        raw_book = signal.get("book", "")
        raw_odds = signal.get("odds", "")
        books_list = [b.strip() for b in str(raw_book).split("/")]
        odds_list  = [o.strip() for o in str(raw_odds).split("/")]

        if len(books_list) >= 2 and len(odds_list) >= 2:
            decimals = [_american_to_decimal(o) for o in odds_list[:2]]
            invs     = [1.0 / d for d in decimals]
            total    = sum(invs) or 1.0
            stakes   = [round(inv / total * 100, 1) for inv in invs]
            diff     = round(100.0 - sum(stakes), 1)
            ...
            play_parts = str(signal.get("play", "")).replace("Arb: ", "").split("+")
            sides      = [p.strip() for p in play_parts] if len(play_parts) >= 2 else ["Side 1", "Side 2"]
            ...
            leg_lines = [
                ...
                f"   Play:  {sides[0]}\n"
                f"   Odds:  `{odds_list[0]}`\n"
                f"   Stake: **{stakes[0]}%** of bankroll",
                f"🔴 **Bet 2:**\n"
                f"   Book:  **{books_list[1]}**\n"
                f"   Play:  {sides[1] if len(sides) > 1 else 'Side 2'}\n"
                f"   Odds:  `{odds_list[1]}`\n"
                f"   Stake: **{stakes[1]}%** of bankroll",
            ]
        else:
    ...
    return (
        ...
        f"{legs_block}\n\n"
        f"💰 Estimated Profit: **{profit_pct}%**\n"
        f"✅ Place BOTH bets at the same time\n"
        f"⏱ {signal.get('timing', 'N/A')}\n"
        f"{SEP}"
    )
...
# ── Health alert rate limit ────────────────────────────────────────────────────
# Prevents the health webhook from being spammed every loop cycle.
# Errors are still logged — only the Discord send is rate-limited.
_last_health_send:     float = 0.0
HEALTH_COOLDOWN_SECS:  int   = 1800   # 30 minutes


def post_health_alert(system: str, error: str):
    global _last_health_send
    now = time.time()
    if now - _last_health_send < HEALTH_COOLDOWN_SECS:
        log.debug("Health alert rate-limited (cooldown %ds remaining): %s — %s",
                  int(HEALTH_COOLDOWN_SECS - (now - _last_health_send)), system, error)
        return
    _last_health_send = now
