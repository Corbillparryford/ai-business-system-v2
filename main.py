...
def _run_content():
    ...
    while True:
        try:
            ...
            # Check bankroll milestones each content cycle
            try:
                from sports.performance_tracker import get_performance
                from content.whop import check_and_post_milestone
                bk_pct = get_performance().get("bankroll_pct", 0.0)
                if bk_pct > 0:
                    check_and_post_milestone(bk_pct)
            except Exception:
                pass
